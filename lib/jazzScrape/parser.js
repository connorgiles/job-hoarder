...
function parseJobs(data) {
  ...
  $('.jobs-list ul li.list-group-item').each(function (i, elem) {
    ...
    jobs.push({
      ...
      title: job.title,
      datePosted: null,
      jobLocation: job.find('.fa-map-marker').parent().text(),
      department: job.find('.fa-sitemap').parent().text(),
      description: null
    })
  });
...
function parseJob(data) {
  ...
  const $ = cheerio.load(data);
  const jobObject = JSON.parse($('script[type="application/ld+json"]').html());

  const url = jobObject.url;
  const idMatch = url.match(/.+apply\/(.+)\/.+/);
  const location = jobObject.jobLocation ? jobObject.jobLocation.address : null;
  ...
  const job = {
    ...
    jobLocation: location ? `${location.addressLocality}, ${location.addressRegion}` : null,
    department: $('li[title="Department"]').text().trim(),
