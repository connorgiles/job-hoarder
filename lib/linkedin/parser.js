...
const parseDetails = body => {
  ...
  const applicationCount = $('.num-applicants__caption')
    .text()
    .trim();

  const posterElement = $('.profile-result-card');
  let poster = undefined;
  if (posterElement.length !== 0) {
    const posterLinkElement = posterElement.find('a');
    const posterName = posterLinkElement.text().trim();
    const posterLink = posterLinkElement.attr('href');
    const posterImage = posterElement.find('img').attr('data-delayed-url');
    const posterHeadline = posterElement
      .find('.profile-result-card__subtitle')
      .text()
      .trim();
    poster = {
      name: posterName,
